...
with open('/home/ringo/catkin_ws/src/lane_laser/Scripts/calibrate_camera.p', 'rb') as f:
...
def laserScan(img,xc,yc,inc,amin,amax):
    ...
    # rsz=cv2.resize(img, (int(width),int(height)), interpolation = cv2.INTER_AREA)
    # rsz[rsz>0]=1
    ranges=np.ones([1,int(2*amax//inc)], dtype=float)*4.0
    for i in range(img.shape[1]): # width - columns - X
        for j in range(img.shape[0]): #height - rows - Y
            if img[j][i]:
                ...
                rangeIndex= (angle + amax)//inc
                if r<ranges[1,int(rangeIndex)]: ranges[1,int(rangeIndex)] = r
    return ranges
...
def talker():
    ...
    rate = rospy.Rate(10)
    cap = cv2.VideoCapture("/home/ringo/catkin_ws/src/lane_laser/Scripts/test_480.mp4")
